...
<?php
	...
	if(isset($_POST['saveBtn'])) {
        ...
        //change old do role        
        $change_old_role_query = "SELECT `nic` FROM `division_officer` WHERE `divi_id` = '{$division_id}' LIMIT 1";
        
		$change_old_role_result = mysqli_query($con,$change_old_role_query);
        
        if(mysqli_num_rows($change_old_role_result)==1) {
            $old_do_nic = mysqli_fetch_assoc($change_old_role_result)['nic'];
            
            $update_old_role = "UPDATE `voter` SET `role` = 'voter' WHERE `nic` = '{$old_do_nic}'";
            $update_old_role_result = mysqli_query($con,$update_old_role);
            
            if ($update_old_role_result) {

                $change_do_query = "UPDATE `division_officer` SET `nic` = '{$nic}', `password` = '{$h_pwd}' WHERE `divi_id` = '{$division_id}'";
                $change_do_result = mysqli_query($con,$change_do_query);
...
<html lang="en">
    <head>
        <title>Change Division Officer | FVS</title>
        <meta charset="utf-8">
            ...
            <main class="page-content">
                <div class="container">
                    ...
                    <div class="row topic">
                        ...
                        <div class="col-md-11">
                            <span class="font-weight-bold"><big>Change Division Officer</big><br><small>Division Officer</small></span>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 form">
                            ...
                            <br><hr><br>
                            <!-- Form -->
                            <form action="admin_changeDivisionOfficer.php" method="post">
                                    ...
                                    <div class="col-sm-7">
                                        <select class="form-control" name="division" required>
                                            <?php
                                                ...
                                                $query = "SELECT d.`id`, d.`name`, dis.`name` AS 'district' FROM `division` d, `district` dis WHERE d.`dist_id` = dis.`id` AND dis.`id` = {$_SESSION['administration_working_at']}";
                                                ...
                                                if (mysqli_num_rows($result_set) >= 1){
                                                    ...
                                                    while($division = mysqli_fetch_assoc($result_set)){
                                                        echo "<option value='".$division['id']."'>".$division['id']." - ".$division['name']." - ".$division['district']."</option>";
                                                    }
                                        ...
                                        <select class="form-control" name="nic" required>
                                            <?php
                                                ...
                                                if (mysqli_num_rows($result_set) >= 1) {
                                                    ...
                                                    while($voters_for_do = mysqli_fetch_assoc($result_set)){
                                                        echo "<option value='".$voters_for_do['nic']."'>".$voters_for_do['nic']." - ".$voters_for_do['name']."</option>";
                                                    }
