...
class pdf extends TCPDF
{
	public function Header() {
		...
		$this->Cell(180, 5, 'Assistant Election Officer List Report', 0, 1, 'C');
	...
	public function Footer() {
		...
		$this->Ln(3);
		$this->Cell(180, 3, '_____________________________________________________________________________________', 0, 1, 'C');
		$this->Cell(180, 3, ' - Report of Assistant Election Officer List from FVS -', 0, 1, 'C');
...
$pdf->SetAuthor('FVS');
$pdf->SetTitle('Assistant Election Officer List Report');
$pdf->SetSubject('');
$pdf->SetKeywords('');
...
$pdf->Cell(45, 7, 'District Name', 0, 0, 'C', 1);
$pdf->Cell(50, 7, 'NIC', 0, 0, 'C', 1);
$pdf->Cell(60, 7, 'Name', 0, 1, 'C', 1);

$query2 = "SELECT a.`dist_id`, d.`name` AS 'd_name', v.`nic`, v.`name` FROM `assistant_election_officer` a, `voter` v, `district` d WHERE v.`nic` = a.`nic` AND a.`dist_id` = d.`id`";
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Ln(2);
    $pdf->Cell(25, 4, $row['dist_id'], 0, 0, 'C');
    $pdf->MultiCell(45, 4, $row['d_name'], 0, 'L', 0, 0, '', '', true);
    $pdf->MultiCell(50, 4, $row['nic'], 0, 'L', 0, 0, '', '', true);
    $pdf->Cell(60, 4, $row['name'], 0, 1, 'L');
...
$pdf->Output('Assistant Election Officer List Report.pdf', 'I');
