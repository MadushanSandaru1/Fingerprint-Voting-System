...
if(isset($_GET['divisionId'])) {
    ...
    header("location:../../../html/administration/admin_electionDistrictResult.php");
}
...
//votes info
if(isset($_GET['divisionId'])) {
    $registered_vote_count_query = "SELECT COUNT(*) AS 'votersCount' FROM `voter` WHERE `is_died` = 0 AND `is_deleted` = 0 AND TIMESTAMPDIFF(YEAR, `b_day`, '".substr($election_info_date_from,0,10)."') >= 18 AND `divi_id` = {$_GET['divisionId']}";
} else if(isset($_GET['districtId'])) {
    $registered_vote_count_query = "SELECT COUNT(v.`nic`) AS 'votersCount' FROM `voter` v, `division` divi, `district` dist WHERE v.`divi_id` = divi.`id` AND divi.`dist_id` = dist.`id` AND v.`is_died` = 0 AND v.`is_deleted` = 0 AND TIMESTAMPDIFF(YEAR, v.`b_day`, '".substr($election_info_date_from,0,10)."') >= 18 AND dist.`id` = {$_GET['districtId']}";
}
...
if(isset($_GET['divisionId'])) {
    $vote_count_query = "SELECT count(*) AS 'declaredTotal', SUM(case when `candidate_id` IS NULL then 1 else 0 end) AS 'rejectedTotal' FROM `vote` WHERE `schedule_id` = {$_GET['scheduleId']} AND `divi_id` = {$_GET['divisionId']}";
} else if(isset($_GET['districtId'])) {
    $vote_count_query = "SELECT count(v.`id`) AS 'declaredTotal', SUM(case when v.`candidate_id` IS NULL then 1 else 0 end) AS 'rejectedTotal' FROM `vote` v, `division` divi WHERE v.`divi_id` = divi.`id` AND v.`schedule_id` = {$_GET['scheduleId']} AND divi.`dist_id` = {$_GET['districtId']}";
}
...
$pdf->Cell(180, 5, 'Election Results - '.$location_name.' '.$location_type.' ('.$location_id.')', 0, 1, 'C');
...
$pdf->SetFillColor(111, 44, 145);
$pdf->SetTextColor(255,255,255);
if($election_info_type == 1) {
    $pdf->Cell(65, 7, 'Party', 0, 0, 'C', 1);
    $pdf->Cell(35, 7, 'Candidate NIC', 0, 0, 'C', 1);
    $pdf->Cell(55, 7, 'Candidate Name', 0, 0, 'C', 1);
    $pdf->Cell(25, 7, 'No. of Votes', 0, 1, 'C', 1);
} else {
    $pdf->Cell(35, 7, 'Candidate NIC', 0, 0, 'C', 1);
    $pdf->Cell(65, 7, 'Candidate Name', 0, 0, 'C', 1);
    $pdf->Cell(55, 7, 'Party', 0, 0, 'C', 1);
    $pdf->Cell(25, 7, 'No. of Votes', 0, 1, 'C', 1);
}

//no of votes group by candidate
if(isset($_GET['divisionId'])) {
...
while ($row = $result->fetch_assoc()) {
    ...
    if($election_info_type == 1) {
        $pdf->MultiCell(65, 4, $row['id']." - ".$row['name_en'], 0, 'L', 0, 0, '', '', true);
        $pdf->Cell(35, 4, $row['nic'], 0, 0, 'C');
        $pdf->MultiCell(55, 4, $row['name'], 0, 'L', 0, 0, '', '', true);
        $pdf->Cell(25, 4, $row['no_of_votes'], 0, 1, 'C');
    } else {
        $pdf->Cell(35, 4, $row['nic'], 0, 0, 'C');
        $pdf->MultiCell(55, 4, $row['name'], 0, 'L', 0, 0, '', '', true);
        $pdf->MultiCell(65, 4, $row['id']." - ".$row['name_en'], 0, 'L', 0, 0, '', '', true);
...
$pdf->Output('Election Results Report '.$location_name.' '.$location_type.'-'.$location_id.'.pdf', 'I');
