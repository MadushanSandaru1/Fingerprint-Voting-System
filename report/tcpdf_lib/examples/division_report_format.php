...
class pdf extends TCPDF
{
	public function Header() {
		...
		$this->Cell(180, 5, 'Division List Report', 0, 1, 'C');
	...
	public function Footer() {
		...
		$this->Ln(3);
		$this->Cell(180, 3, '_____________________________________________________________________________________', 0, 1, 'C');
		$this->Cell(180, 3, ' - Report of Division List from FVS -', 0, 1, 'C');
...
$pdf->SetAuthor('FVS');
$pdf->SetTitle('Division List Report');
$pdf->SetSubject('');
$pdf->SetKeywords('');
...
$pdf->Cell(50, 7, 'District', 0, 0, 'C', 1);
$pdf->Cell(60, 7, 'Province', 0, 1, 'C', 1);

$query2 = "SELECT dv.`id` as 'id', p.`id` as 'p_id', p.`name` as 'p_name', d.`name` as 'd_name', d.`id` as 'd_id', dv.`name` as 'dv_name' from `province` p, `district` d, `division` dv WHERE p.`id` = d.`prov_id` AND d.`id` = dv.`dist_id`";
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->MultiCell(45, 4, $row['dv_name'], 0, 'L', 0, 0, '', '', true);
    $pdf->MultiCell(50, 4, "(".$row['d_id'].") ".$row['d_name'], 0, 'L', 0, 0, '', '', true);
    $pdf->Cell(60, 4, "(".$row['p_id'].") ".$row['p_name'], 0, 1, 'L');
...
$pdf->Output('Division List Report.pdf', 'I');
